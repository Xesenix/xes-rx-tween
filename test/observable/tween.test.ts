...
describe("TweenObservable", () => {
	describe("constructor", () => {
		...
		testCases.forEach(({ delay, duration }) => {
			it(`should end execution after ${delay +
				duration}ms including ${delay}ms delay and return growing values between 0 and 1`, done => {
	...
	describe("create", () => {
		...
		testCases.forEach(({ duration, startValue, endValue }) => {
			it(`should end execution after ${duration}ms duration and return values from ${startValue} to ${endValue}`, done => {
				const startTime = Date.now();
				let lastValue = startValue;

				const tween = TweenObservable.create(duration, startValue, endValue);
